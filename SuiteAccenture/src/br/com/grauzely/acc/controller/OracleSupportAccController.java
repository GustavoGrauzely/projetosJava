...
import br.com.grauzely.acc.util.ValidarCampos;
...
public class OracleSupportAccController implements Initializable {
	...
	private Connection connection = (Connection) ConnectionUtil.connectdb();
	private int id_gerar;
	...
	public void adicionar() {
		if (ValidarCampos.checkEmptyFields(demanda, numeroSR, status, usuarios,
				descricao, data)) {
	...
	public void editar() {
		...
		if (ValidarCampos.checkEmptyFields(demanda, numeroSR, status, usuarios,
				descricao, data)) {
			if (selectedIndex >= 0) {
				tabelaSR.getItems().get(selectedIndex)
						.setDemanda(demanda.getText());
				tabelaSR.getItems().get(selectedIndex)
						.setNumeroSR(numeroSR.getText());
				tabelaSR.getItems().get(selectedIndex)
						.setStatus(status.getValue());
				tabelaSR.getItems().get(selectedIndex)
						.setUsuario(usuarios.getValue());
				tabelaSR.getItems().get(selectedIndex)
						.setDescricao(descricao.getText());
				tabelaSR.getItems()
						.get(selectedIndex)
						.setData(
								FORMATADOR.format(DateUtil.asDate(data
										.getValue())));
				demanda.setText("");
				numeroSR.setText("");
				status.getSelectionModel().select("Work in Progress");
	...
	public void gerarRelatorio() {
		id_gerar = serviceRequestsDao.buscarUltimaGeracao();
		try {
			...
			infoBox("Aconteceu algum erro na geração!", "Error!", null);
		}
	}

	public void imprimir() throws JRException {
		String url = "C:\\Users\\g.da.silva.alves\\workspace\\SuiteAccenture\\src\\br\\com\\grauzely\\acc\\relatorios\\SRsReports.jrxml";
		JasperReport jasperReport = JasperCompileManager.compileReport(url);
		Map<String, Object> parametros = new HashMap<>();
		parametros.put("id_geracao", id_gerar);
